...
def reconcile(previous, found):
    ...
    for ref in found['references']:
        ...
        elif previous.get('links_reviewed') and old_counts[text] == new_counts[text] == 1 and len(candidates) == 1:
            old = candidates[0]
            for key in ('target', 'manual', 'scope_manual', 'link_start', 'link_end', 'link_text', 'scope_review'):
