...
function App() {
  ...
  return <>
    ...
    {!p && !loaded ? <main className="empty"><h1>{online === false ? 'Ожидаем запуск приложения' : error ? 'Не удалось открыть сохранённую работу' : 'Открываем сохранённую работу…'}</h1>{error && online ? <button onClick={() => setRetry(r => r+1)}>Повторить загрузку</button> : null}</main> : !p ? <main className="empty"><h1>Подготовьте первую подачу</h1><p>Создайте локальный проект или откройте вымышленные учебные документы для проверки полного процесса.</p><div className="actions"><button className="primary" disabled={Boolean(busy)} onClick={() => setCreating(true)}>Создать подачу</button><button disabled={Boolean(busy)} onClick={() => run('Создаём учебные документы…', async () => { accept(await api('/demo', {})); await refreshList(); })}>Открыть учебный комплект</button></div><p className="muted">Работа с приложениями доступна без основного Word-документа.</p></main> : <>
      ...
      {tab === 'links' ? <Links error={error} p={p} busy={operationBusy} mutate={mutate} upload={upload} focusRef={focusRef}/> : null}
...
function Links({p,busy,mutate,upload,focusRef,error}) {
  ...
  return <section className="links-page"><div className="section-heading"><div><h1>Ссылки в существующих сносках</h1><p className="muted">{p.main?.name || 'Загрузите основной DOCX. Приложения могут быть уже оформлены.'}</p></div><div className="actions"><Upload accept=".docx" disabled={Boolean(busy)} onFiles={f=>upload(f,'main')}>{p.main?'Заменить DOCX':'Загрузить DOCX'}</Upload><button className="primary" disabled={Boolean(busy||!p.main)} onClick={()=>mutate('/scan',{},'Сопоставление обновлено. Проверьте все сноски.')}>Сопоставить ссылки</button></div></div>
    ...
    <table className="links-table"><thead><tr><th>Сноска*</th><th>Текст гиперссылки</th><th>Соответствующий файл</th><th>Сопоставление</th></tr></thead><tbody>{p.references.map(r=><tr key={r.key} id={'ref-'+r.key} className={focusRef===r.key?'selected':''}><td>{r.footnote}</td><td><span>{r.link_text??r.mention}</span>{r.managed_current_text?<small className="pending">В проекте: {r.managed_current_text}. Обновите ссылку в панели Word или подтвердите текущий текст.</small>:null}{r.scope_review?<small className="pending">Проверьте текст и границы ссылки.</small>:null}<button disabled={Boolean(busy||!p.scanned)} onClick={()=>setScope(r.key)}>Изменить границы</button></td><td><select aria-label={`Файл для ${r.mention} в сноске ${r.footnote}`} value={r.target || ''} disabled={Boolean(busy||!p.scanned)} onChange={e=>mutate('/references',{key:r.key,target:e.target.value,all_same:allSame})}><option value="" disabled>{r.candidates.length>1?'Несколько кандидатов':'Не найден — выберите файл'}</option>{p.documents.map(d=><option key={d.id} value={d.id}>{d.identifier ? d.identifier+' · ' : ''}{d.title}</option>)}</select></td><td><span className={'status '+(r.target?'ready':'problem')}>{r.manual?'Ручной выбор':r.target?'Точное совпадение':'Требует решения'}</span></td></tr>)}</tbody></table>
    ...
    <div className="links-bottom"><div><h2>Проверьте полный текст сносок</h2><p className="muted">Неизвестные названия нельзя надёжно обнаружить автоматически. Добавьте пропущенное упоминание ниже.</p>{p.footnotes.map(f=><p className="footnote" key={`${f.fid}:${f.paragraph}`}><b>{f.footnote}</b><span>{f.text}</span></p>)}
      ...
      <div><h2>Подтверждение</h2><p>Проверьте каждую сноску, включая упоминания без номера приложения. Подтверждение означает, что все нужные ссылки найдены и ведут к выбранным документам.</p><button className="primary" disabled={Boolean(busy||!p.main||!p.scanned||p.references.some(r=>!r.target)||p.links_reviewed)} onClick={()=>mutate('/references/confirm',{},'Сопоставление проверено')}>{p.links_reviewed?'Сопоставление подтверждено':'Все сноски проверены'}</button><p className="muted">Текст, состав и нумерация сносок не переписываются. Существующие ссылки на выбранных упоминаниях обновляются.</p>
