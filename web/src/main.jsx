...
import SubmissionFolders, {SubmissionTree} from './SubmissionFolders';
...
import WordWorkflow, {EditionReport, ExportHistory} from './WordWorkflow';
...
function App() {
  ...
  const [checked, setChecked] = useState([]), [sort, setSort] = useState('added'), [batch, setBatch] = useState(null), [foldersOpen,setFoldersOpen]=useState(false);
  ...
  async function upload(files, kind = 'original', did, mode = 'prepare') {
    return run('Загружаем файлы…', async () => {
      let current;
      const failures = [];
      for (let i=0; i<files.length; i++) {
        setBusy(`Загрузка ${i+1} из ${files.length}…`);
        const query = new URLSearchParams({name: files[i].name, kind, mode, ...(did ? {did} : {})});
      ...
      else setNotice(mode === 'passthrough' ? 'Готовые PDF добавлены без обработки. Можно перейти к ссылкам в Word.' : 'Файлы сохранены. Исходники не изменены.');
      if (mode === 'passthrough' && current) setSelected(current.documents.at(-1)?.id);
    });
  }
  async function useReadyPdfs() {
    const project = await mutate('/ready-pdfs', {document_ids: checked}, 'Выбранные PDF будут использованы целиком, без изменений. Можно перейти к ссылкам в Word.');
    if (project) setChecked([]);
  }
  const operationBusy = busy || (online !== true ? 'Нет связи' : '');
  const disabled = Boolean(operationBusy || dirty);
  return <>
    <div inert={Boolean(batch || creating || translation || backupOpen)}>
    <header><div className="brand">Buzko Legal <span>Приложения</span></div><div className="project-switch"><label htmlFor="project">Проект</label><select id="project" aria-label="Проект" value={p?.id || ''} disabled={disabled} onChange={e => run('Открываем…', async () => { accept(await api('/projects/'+e.target.value)); setSelected(null); setSearch(''); setTab('docs'); })}><option value="" disabled>Выберите подачу</option>{projects.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select><button disabled={disabled} onClick={() => setCreating(true)}>Новая</button><button disabled={disabled} onClick={() => setBackupOpen(true)}>Архив проекта</button></div><span className="local"><i/>{online === false ? 'Нет связи' : 'Локально'}</span></header>
    <nav aria-label="Этапы подготовки">{[['docs','Документы'],['links','Ссылки в Word'],['word','Работа в Word'],['export','Проверка и экспорт']].map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} disabled={Boolean(busy)} onClick={() => guard(() => setTab(key))}>{label}{key==='export' && p?.issues.length ? <small>{p.issues.length}</small> : null}</button>)}<span className="save-state">{dirty ? 'Есть несохранённые настройки' : p ? 'Работа сохранена на компьютере' : 'Первый сквозной этап'}</span></nav>
    ...
    {p?.schema_backup ? <p className="success-note">Перед обновлением формата сохранена резервная копия в папке данных: {p.schema_backup}</p> : null}
    ...
    {!p && !loaded ? <main className="empty"><h1>{online === false ? 'Ожидаем запуск приложения' : error ? 'Не удалось открыть сохранённую работу' : 'Открываем сохранённую работу…'}</h1>{error && online ? <button onClick={() => setRetry(r => r+1)}>Повторить загрузку</button> : null}</main> : !p ? <main className="empty"><h1>Подготовьте первую подачу</h1><p>Создайте локальный проект или откройте вымышленные учебные документы для проверки полного процесса.</p><div className="actions"><button className="primary" disabled={Boolean(busy)} onClick={() => setCreating(true)}>Создать подачу</button><button disabled={Boolean(busy)} onClick={() => run('Создаём учебные документы…', async () => { accept(await api('/demo', {})); await refreshList(); })}>Открыть учебный комплект</button></div><p className="muted">Работа с приложениями доступна без основного Word-документа.</p></main> : <>
      {tab === 'docs' ? <main className="workbench">
        <section className="document-list"><div className="section-heading"><h1>Документы</h1><Upload multiple disabled={disabled} onFiles={upload}>Добавить PDF</Upload></div>
          <div className="ready-pdf-upload"><Upload multiple disabled={disabled} onFiles={files=>upload(files,'original',undefined,'passthrough')}>Добавить готовые PDF</Upload><p className="muted">Уже оформлены? Загрузите сразу все файлы: они войдут в комплект целиком, без новых штампов и подтверждения каждого PDF.</p>{p.documents.some(d=>d.mode==='passthrough') ? <button disabled={disabled} onClick={()=>setTab('links')}>К ссылкам в Word →</button> : null}</div>
          <div className="folder-shortcut"><button disabled={disabled||!p.documents.length} onClick={()=>{setError('');setFoldersOpen(true);}}>Папки Submission</button><p className="muted">Распределите PDF по своим папкам и подпапкам. Ссылки будут учитывать выбранную структуру.</p></div>
      ...
      {tab === 'word' ? <WordWorkflow key={p.id} p={p} api={api} mutate={mutate} busy={operationBusy} onDirty={setDirty}/> : null}
      ...
      {tab === 'export' ? <section className="export-page"><div><h1>Проверка перед сборкой</h1><p className="muted">В комплект войдут все загруженные приложения, в том числе не упомянутые в Word.</p></div>
        <div className="export-grid"><div><h2>{p.issues.length ? `Требуют внимания: ${p.issues.length}` : 'Блокирующих ошибок нет'}</h2>{p.issues.length ? <ul className="issues">{p.issues.map((issue, i) => <li key={i}><span className="issue-dot">!</span><div>{issue.message}<small>{p.documents.find(d => d.id === issue.document)?.title}</small></div><button disabled={Boolean(busy)} onClick={() => guard(() => { if (issue.document) { setSelected(issue.document); setTab('docs'); } else { setFocusRef(issue.reference); setTab('links'); } })}>Перейти</button></li>)}</ul> : <p className="success-note">Материалы проверены в инструменте. После экспорта проверьте комплект в Word и PDF-просмотрщике перед подачей.</p>}
          <button className="primary" disabled={Boolean(operationBusy || dirty || p.issues.length)} onClick={() => run('Собираем подачу…', async () => { const blob = await api(`/projects/${p.id}/export`, {}, {blob: true}); saveBlob(blob, 'Submission.zip'); accept(await api('/projects/'+p.id)); setNotice('ZIP собран. Распакуйте весь комплект, сохранив структуру папок.'); })}>Собрать подачу · ZIP</button><p className="muted">{p.main?'Основной DOCX и PDF войдут в ZIP вместе с приложениями. Если PDF ещё не подготовлен, сборка создаст его автоматически.':'В ZIP войдут подготовленные приложения.'}</p>{p.main?<MainPdf key={p.id} p={p} api={api} run={run} disabled={Boolean(operationBusy||dirty)}/>:null}</div>
        <div><h2>Оформление подачи</h2><StyleForm style={p.style} onDirty={setStyleDirty} busy={operationBusy} onSave={style => mutate('/style', style)}/><p className="muted">В полях страницы штампы сохраняют размер и положение содержимого. Если места нет, измените отступы или выберите дополнительную полосу. После изменения оформления приложения требуют повторной проверки.</p><h2>Структура комплекта</h2><button disabled={disabled||!p.documents.length} onClick={()=>{setError('');setFoldersOpen(true);}}>Папки Submission</button><SubmissionTree documents={p.documents} main={p.main}/></div></div>
      <ExportHistory pid={p.id} revision={revision} api={api} run={run} disabled={disabled} saveBlob={saveBlob}/>
      </section> : null}
    </>}
    </div>
    ...
    {foldersOpen&&p ? <SubmissionFolders key={p.id} p={p} busy={operationBusy} error={error} onClose={()=>{setFoldersOpen(false);setError('');}} onPreview={request=>run('Проверяем папки…',()=>api(`/projects/${p.id}/batch/preview`,request))} onApply={(request,token)=>run('Сохраняем структуру…',async()=>{accept(await api(`/projects/${p.id}/batch/apply`,{request,token}));setFoldersOpen(false);setNotice('Структура сохранена. Соберите новый ZIP — ссылки будут вести в выбранные папки.');})}/> : null}
    ...
    {backupOpen ? <ProjectBackup p={p} api={api} onClose={()=>setBackupOpen(false)} onRestored={async project=>{accept(project);await refreshList();setSelected(null);setTab('docs');setBackupOpen(false);setNotice('Проект восстановлен. Можно продолжить работу.');}}/> : null}
...
const FIELDS = ['title','short_title','prefix','number','designation','filename','folder','language','mode','original_label','translation_label','aliases','filename_mode'];
...
function Inspector({p, doc, busy, onDirty, mutate, upload, onFormat, onTranslation}) {
  ...
  return <aside className="inspector"><h2>Настройки приложения</h2><form onSubmit={e => { e.preventDefault(); const original=draftFrom(doc);const changes=Object.fromEntries(FIELDS.filter(k=>draft[k]!==original[k]).map(k=>[k,draft[k]]));if('number' in changes)changes.number=draft.number===''?null:Number(draft.number);if('aliases' in changes)changes.aliases=draft.aliases.split('\n').map(x=>x.trim()).filter(Boolean);mutate(`/documents/${doc.id}`, changes); }}><fieldset disabled={Boolean(busy)}>
    {field('Название документа','title')}{field('Короткое название для сносок (необязательно)','short_title')}<div className="pair">{field('Префикс (необязателен)','prefix')}{field('Номер (необязателен)','number','number')}</div>
  ...
  {doc.mode === 'prepare' ? <section className="format-origin"><h2>Оформление документа</h2><p className="muted">{doc.effective_format.font} · {doc.effective_format.size} pt · отступ {doc.effective_format.margin} pt</p><details><summary>Откуда взяты настройки</summary>{Object.entries({font:'Шрифт',size:'Размер',margin:'Отступ',top:'Отступ сверху',stamp_mode:'Размещение',designation:'Обозначение',original_label:'Пометка оригинала',translation_label:'Пометка перевода'}).map(([k,label])=><p className="muted" key={k}>{label}: {{project:'подача',group:'группа',document:'этот документ'}[doc.format_sources[k]]}</p>)}</details><button disabled={Boolean(busy||dirty)} onClick={onFormat}>Изменить оформление документа</button></section> : null}
...
function Preview({p, doc, revision, disabled, onSave, onError}) {
  ...
  return <section className="preview"><h2>Просмотр документа</h2><div className="preview-tabs">{[['original','Оригинал'],...(doc.translation ? [['translation','Перевод']] : []),['result','Результат']].map(([value,label])=><button key={value} className={part===value?'active':''} onClick={()=>{setPart(value);setPage(1);setDrawing(false);}}>{label}</button>)}</div>
    ...
    </details>{doc[part]?.text_pages.some(x=>!x) ? <p className="pending">Есть страницы без доступного текста. OCR и автоматический перевод сканов пока не поддерживаются.</p> : null}</div> : <p className="muted inset">{doc.mode==='passthrough'?'Без обработки: весь исходный PDF сохранится без изменений.':'Порядок: перевод, затем оригинал. Номер здесь — страница сборки.'}</p>}
  </section>;
}

function Links({p,busy,mutate,upload,focusRef,error,clearError}) {
  const [allSame,setAllSame]=useState(true),[para,setPara]=useState(''),[mention,setMention]=useState(''),[scope,setScope]=useState(null),[adding,setAdding]=useState(null),[addedKey,setAddedKey]=useState(null);
  const scoped=p.references.find(r=>r.key===scope);
  useEffect(()=>{if(addedKey)document.getElementById('ref-'+addedKey)?.scrollIntoView({block:'center'});},[addedKey]);
  ...
  return <section className="links-page"><div className="section-heading"><div><h1>Ссылки в существующих сносках</h1><p className="muted">{p.main?.name || 'Загрузите основной DOCX. Приложения могут быть уже оформлены.'}</p></div><div className="actions"><Upload accept=".docx" disabled={Boolean(busy)} onFiles={f=>upload(f,'main')}>{p.main?'Заменить DOCX':'Загрузить DOCX'}</Upload><button className="primary" disabled={Boolean(busy||!p.main)} onClick={()=>mutate('/scan',{},'Сопоставление обновлено. Проверьте все сноски.')}>Сопоставить ссылки</button></div></div>
    <EditionReport report={p.edition_report}/>
    {p.scanned === false ? <p className="pending">Основной документ или реквизиты изменились. Повторите сопоставление.</p> : null}
    <button disabled={Boolean(busy||!p.scanned||!p.footnotes.length)} onClick={()=>{clearError();setAdding(0);}}>Добавить ссылку из сноски</button>
    ...
    <table className="links-table"><thead><tr><th>Сноска*</th><th>Текст гиперссылки</th><th>Соответствующий файл</th><th>Сопоставление</th></tr></thead><tbody>{p.references.map(r=><tr key={r.key} id={'ref-'+r.key} className={focusRef===r.key||addedKey===r.key?'selected':''}><td>{r.footnote}</td><td><span>{r.link_text??r.mention}</span>{r.scope_review&&!r.keep_original?<small className="pending">Проверьте границу: уточнение страницы или пункта не распознано.</small>:null}<button disabled={Boolean(busy||!p.scanned||r.keep_original)} onClick={()=>{clearError();setScope(r.key);}}>Изменить границы</button><button disabled={Boolean(busy||!p.scanned)} title="Убрать ошибочное совпадение из списка; текст сноски сохранится" onClick={()=>mutate('/references/exclude',{key:r.key},'Упоминание исключено. Текст сноски сохранён.')}>Не считать ссылкой</button></td><td><select aria-label={`Файл для ${r.mention} в сноске ${r.footnote}`} value={r.keep_original ? '__keep_original__' : r.target || ''} disabled={Boolean(busy||!p.scanned)} onChange={e=>mutate('/references',{key:r.key,target:e.target.value==='__keep_original__'?null:e.target.value||null,keep_original:e.target.value==='__keep_original__',all_same:allSame})}><option value="">{r.candidates.length>1?'Несколько кандидатов':'Не найден — выберите решение'}</option><option value="__keep_original__">Без файла — оставить как есть</option>{p.documents.map(d=><option key={d.id} value={d.id}>{d.identifier ? d.identifier+' · ' : ''}{d.title}</option>)}</select></td><td><span className={'status '+(r.target||r.keep_original?'ready':'problem')}>{r.keep_original?'Без файла · без изменений':!r.target?'Требует решения':r.manual?'Ручной выбор':'Точное совпадение'}</span></td></tr>)}</tbody></table>
    <p className="muted">*Порядок сносок в тексте. Нестандартная нумерация и перезапуск по разделам требуют проверки в Word.</p>
    {p.excluded_references?.length ? <details className="excluded-references"><summary>Исключённые упоминания ({p.excluded_references.length})</summary><p className="muted">Текст остался в Word. Эти совпадения не мешают назначать границы других ссылок.</p>{p.excluded_references.map(r=><p key={r.key}>Сноска {r.footnote}: {r.mention} <button disabled={Boolean(busy||!p.scanned)} onClick={()=>mutate('/references/exclude',{key:r.key,restore:true},'Упоминание восстановлено')}>Восстановить</button></p>)}</details>:null}
    <div className="links-bottom"><div><h2>Проверьте полный текст сносок</h2><p className="muted">Для пропущенного названия нажмите «Добавить ссылку» рядом со сноской и выделите нужный текст.</p>{p.footnotes.map(f=><p className="footnote" key={`${f.fid}:${f.paragraph}`}><b>{f.footnote}</b><span>{f.text}<button disabled={Boolean(busy||!p.scanned)} onClick={()=>{clearError();setAdding(p.footnotes.indexOf(f));}}>Добавить ссылку</button></span></p>)}
    ...
    {adding!==null?<ReferenceAdd footnotes={p.footnotes} documents={p.documents} initial={adding} busy={busy} error={error} onClose={()=>setAdding(null)} onSave={async body=>{const result=await mutate('/references/add',body,'Ссылка добавлена. Проверьте сопоставление.');if(result)setAddedKey(`${body.fid}:${body.paragraph}:${body.start}:${body.end}`);return result;}}/>:null}
