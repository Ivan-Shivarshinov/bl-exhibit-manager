...
const TITLES = {font:'Шрифт',size:'Размер, pt',margin:'Отступ по горизонтали, pt',designation:'Обозначение',original_label:'Пометка оригинала',translation_label:'Пометка перевода'};
...
export default function BatchDialog({p, ids, initialAction, busy, error, onClose, onPreview, onApply}) {
  ...
  function input(k) {
    const props={value:values[k],disabled:Boolean(busy||reset||!enabled.includes(k)),onChange:e=>change(()=>setValues({...values,[k]:['size','margin'].includes(k)?Number(e.target.value):e.target.value}))};
    ...
    return <input aria-label={'Массовое: '+TITLES[k]} type={['size','margin'].includes(k)?'number':'text'} {...props}/>;
  }
  return <div className="modal-backdrop"><section className="batch-modal" role="dialog" aria-modal="true" aria-label="Массовые изменения">
    ...
    {action==='organize'?<div className="batch-settings">
      <div><label className="checkline"><input type="checkbox" checked={number} disabled={Boolean(busy)} onChange={e=>change(()=>setNumber(e.target.checked))}/>Назначить номера выбранным документам без номера</label><div className="pair"><label className="field"><span>Префикс новых номеров</span><input aria-label="Префикс новых номеров" disabled={Boolean(busy||!number)} value={prefix} onChange={e=>change(()=>setPrefix(e.target.value))}/></label><label className="field"><span>Начальный номер</span><input aria-label="Начальный номер" type="number" min="1" disabled={Boolean(busy||!number)} value={start} onChange={e=>change(()=>setStart(e.target.value))}/></label></div><p className="muted">Уже назначенные номера и префиксы сохраняются. Занятые номера будут пропущены. Нумерация новых документов следует порядку строк предпросмотра.</p></div>
